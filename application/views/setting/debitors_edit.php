...
<h1 style="margin:75px 0px 20px 35px;width:100%; font-size:36px;">Payment Ledgers</h1>
<?php
...
<main class="row" style="margin:0px 20px">
    ...
    <div class="col-lg-6 col-sm-12">
        <div class="card" style="background-color:#138575">
            <div class="card-body setting-card-body pb-0">
                <a href="<?php echo base_url('setting/'.$accounting.'/debitors'); ?>" type="button" class="btn btn-success setting-btn" data-card-widget="collapse" data-toggle="tooltip" title="Add">Add</a>
                <p class="lead text-white setting-lead">Setting</p>
            </div>
        </div>
        <?php echo form_open('setting/visma_debitor/update') . "\r\n"; ?>
        ...
        <input type="hidden" name="id" value="<?php echo $debitor_data->id; ?>">
            ...
            <select name="external_id" id="external_id" class="form-control select2" required>
                ...
                if (isset($external_debitors) && is_array($external_debitors)) {
                    foreach ($external_debitors as $debtor) {
                        ...
                        $sel = ($debitor_data->external_id == trim($parts[0]) ? 'selected' : '');
                ?>
            ...
            <select name="payment_type" id="payment_type" class="form-control select2">
                ...
                if (isset($payment_types) && is_array($payment_types)) {
                    foreach ($payment_types as $key => $type) {
                        $sel = ($debitor_data->payment_type == $key ? 'selected' : ''); ?>
                        <option value="<?php echo $key ?>" <?php echo $sel ?>><?php echo $type ?></option>
                <?php  }
                } ?>
            </select>
        </div>
