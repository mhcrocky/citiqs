<div class="main-wrapper main-wrapper-contact" style="text-align:center">

	<div class="col-half background-yankee div-only-mobile height-25">
		<div class="flex-column align-start "><!-- width-650 -->
			<div style="text-align:left; font-family: caption-light">
				<h2 style="font-family: caption-light">
					YOUR DONATION IS HIGHLY APPRECIATED! </h2>
				<div class="heading mb-35" style="font-size: larger">
				</div>
			</div>
		</div>
	</div>

	<div class="col-half background-purple-light timeline-content">
		<div class="pricing-block background-orange-light">
			<div class="pricing-block-body">
				<div class="pricing-first" style="font-family: caption-bold">
					<h2 style="font-family: caption-bold">CUP OF COFFEE</h2>
					<img src="<?php echo $this->baseUrl; ?>assets/home/images/coffee.png" alt="tiqs" width="150" height="auto" />
					<h6 style="font-family: caption-bold">THANKS!</h6>
					<h1 style="font-family:caption-bold ">€ <?php echo $coffee; ?></h1>
					<p style="font-family: caption-light">WE REALLY APPRECIATE YOU GETTING US ONE!</p>
				</div>
			</div><!-- end pricing block body -->

			<div class="pricing-block-footer">
				<a data-brackets-id="2918" href="<?php echo base_url(); ?>pay424/37" class="button button-orange">SELECT</a>
			</div>

			<div class="mt-50" align="center" style="text-align:center; margin-bottom: 20px">
				<div style="text-align:center; margin-bottom: 10px">
					<h2 class="heading">
						<?=$this->language->line("CHECK424-TELL001",'TELL A FRIEND');?>
					</h2>
				</div>

				<a style="margin-right: 10px" href="whatsapp://send?text=Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" data-action="share/whatsapp/share" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on whatsapp">
					<i class="fa fa-whatsapp" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="https://www.facebook.com/sharer/sharer.php?u=https://check424.com&t=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Facebook">
					<i class="fa fa-facebook" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="https://twitter.com/share?url=https://check424.com&text=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out..." onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Twitter">
					<i class="fa fa-twitter" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="mailto:?subject=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out.&body=This is handy! please check this out, and tell your friends and bars, cafe's and restaurants. https://check424.com" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Mail">
					<i class="fa fa-envelope" style="font-size:48px;color:white"></i>
				</a>
			</div>

		</div><!-- end pricing block -->
		<div class="pricing-block background-orange">
			<div class="pricing-block-body">
				<div class="pricing-first" style="font-family: caption-bold">
					<h2 style="font-family: caption-bold">COFFEE AND CROISSANT</h2>
					<img src="<?php echo $this->baseUrl; ?>assets/home/images/coffeeandcroissant.png" alt="tiqs" width="150" height="auto" />
					<h6 style="font-family: caption-bold">SOMETHING HOT WITH A BITE...</h6>
					<h1 style="font-family: caption-bold">€ <?php echo $coffeeandcroissant; ?></h1>
					<p style="font-family: caption-light">THIS MORNING SNACK KEEPS US FOCUSSED....</p>
				</div>
			</div><!-- end pricing block body -->
			<div class="pricing-block-footer">
				<a data-brackets-id="2918" href="<?php echo base_url(); ?>pay424/38" class="button button-orange"><img src="pay.png" alt="">SELECT</a>
			</div>

			<div class="mt-50" align="center" style="text-align:center; margin-bottom: 20px">
				<div style="text-align:center; margin-bottom: 10px">
					<h2 class="heading">
						<?=$this->language->line("CHECK424-TELL001",'TELL A FRIEND');?>
					</h2>
				</div>

				<a style="margin-right: 10px" href="whatsapp://send?text=Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" data-action="share/whatsapp/share" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on whatsapp">
					<i class="fa fa-whatsapp" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="https://www.facebook.com/sharer/sharer.php?u=https://check424.com&t=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Facebook">
					<i class="fa fa-facebook" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="https://twitter.com/share?url=https://check424.com&text=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out..." onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Twitter">
					<i class="fa fa-twitter" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="mailto:?subject=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out.&body=This is handy! please check this out, and tell your friends and bars, cafe's and restaurants. https://check424.com" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Mail">
					<i class="fa fa-envelope" style="font-size:48px;color:white"></i>
				</a>
			</div>
		</div><!-- end pricing block -->

		<div class="pricing-block background-purple-light">
			<div class="pricing-block-body">
				<div class="pricing-first" style="font-family: caption-bold">
					<h2 style="font-family: caption-bold">LUNCH</h2>
					<img src="<?php echo $this->baseUrl; ?>assets/home/images/lunch.png" alt="tiqs" width="150" height="auto" />
					<h6 style="font-family: caption-bold">WE TAKE OUR SANDWICHES AS LUNCH BEHIND THE SCREEN...</h6>
					<h1 style="font-family: caption-bold">€ <?php echo $lunch; ?></h1>
					<p style="font-family: caption-light">LUNCH IS LIKE, I AM HUNGRY BUT I CANNOT LEAVE THE DESK.....</p>
				</div>
			</div><!-- end pricing block body -->
			<div class="pricing-block-footer">
				<a data-brackets-id="2918" href="<?php echo base_url(); ?>pay424/39" class="button button-orange">SELECT</a>
			</div>

			<div class="mt-50" align="center" style="text-align:center; margin-bottom: 20px">
				<div style="text-align:center; margin-bottom: 10px">
					<h2 class="heading">
						<?=$this->language->line("CHECK424-TELL001",'TELL A FRIEND');?>
					</h2>
				</div>

				<a style="margin-right: 10px" href="whatsapp://send?text=Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" data-action="share/whatsapp/share" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on whatsapp">
					<i class="fa fa-whatsapp" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="https://www.facebook.com/sharer/sharer.php?u=https://check424.com&t=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Facebook">
					<i class="fa fa-facebook" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="https://twitter.com/share?url=https://check424.com&text=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out..." onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Twitter">
					<i class="fa fa-twitter" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="mailto:?subject=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out.&body=This is handy! please check this out, and tell your friends and bars, cafe's and restaurants. https://check424.com" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Mail">
					<i class="fa fa-envelope" style="font-size:48px;color:white"></i>
				</a>
			</div>
		</div><!-- end pricing block -->

		<div class="pricing-block background-yellow">
			<div class="pricing-block-body">
				<div class="pricing-first" style="font-family: caption-bold">
					<h2 style="font-family: caption-bold">DINNER</h2>
					<img src="<?php echo $this->baseUrl; ?>assets/home/images/dinner.png" alt="tiqs" width="150" height="auto" />
					<h6 style="font-family: caption-bold">KEEPS US RUNNING ALL NIGHT</h6>
					<h1 style="font-family: caption-bold">€ <?php echo $dinner; ?></h1>
					<p style="font-family: caption-light">WE REALLY NEED TO STICK TO OUR DIET, HOPE YOU APPRECIATE OUR WORK. ....</p>
				</div>
			</div><!-- end pricing block body -->
			<div class="pricing-block-footer">
				<a data-brackets-id="2918" href="<?php echo base_url(); ?>pay424/40" class="button button-orange">SELECT</a>
			</div>

			<div class="mt-50" align="center" style="text-align:center; margin-bottom: 20px">
				<div style="text-align:center; margin-bottom: 10px">
					<h2 class="heading">
						<?=$this->language->line("CHECK424-TELL001",'TELL A FRIEND');?>
					</h2>
				</div>

				<a style="margin-right: 10px" href="whatsapp://send?text=Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" data-action="share/whatsapp/share" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on whatsapp">
					<i class="fa fa-whatsapp" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="https://www.facebook.com/sharer/sharer.php?u=https://check424.com&t=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Facebook">
					<i class="fa fa-facebook" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="https://twitter.com/share?url=https://check424.com&text=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out..." onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Twitter">
					<i class="fa fa-twitter" style="font-size:48px;color:white"></i>
				</a>

				<a style="margin-right: 10px" href="mailto:?subject=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out.&body=This is handy! please check this out, and tell your friends and bars, cafe's and restaurants. https://check424.com" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Mail">
					<i class="fa fa-envelope" style="font-size:48px;color:white"></i>
				</a>
			</div>
		</div><!-- end pricing block -->

	</div>

	<div class="col-half background-yankee height-100">
		<div class="flex-column align-start "><!-- width-650 -->
			<div style="text-align:left; font-family: caption-light">
				<h2 style="font-family: caption-light">
					YOUR DONATION IS HIGHLY APPRECIATED! </h2>
				<div class="heading mb-35" style="font-size: larger">

				<div class="mt-50" align="center" style="text-align:center; margin-bottom: 20px">
					<div style="text-align:center; margin-bottom: 10px">
						<h2 class="heading">
							<?=$this->language->line("CHECK424-TELL001",'TELL A FRIEND');?>
						</h2>
					</div>

					<a style="margin-right: 10px" href="whatsapp://send?text=Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" data-action="share/whatsapp/share" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on whatsapp">
						<i class="fa fa-whatsapp" style="font-size:48px;color:white"></i>
					</a>

					<a style="margin-right: 10px" href="https://www.facebook.com/sharer/sharer.php?u=https://check424.com&t=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out... https://check424.com" onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Facebook">
						<i class="fa fa-facebook" style="font-size:48px;color:white"></i>
					</a>

					<a style="margin-right: 10px" href="https://twitter.com/share?url=https://check424.com&text=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out..." onclick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Twitter">
						<i class="fa fa-twitter" style="font-size:48px;color:white"></i>
					</a>

					<a style="margin-right: 10px" href="mailto:?subject=Very handy for restaurants, bars, Visitors Check for 24Hours to avoid queuing, check-in check-out.&body=This is handy! please check this out, and tell your friends and bars, cafe's and restaurants. https://check424.com" onClick="javascript:window.open(this.href, '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=300,width=600');return false;" target="_blank" title="Share on Mail">
						<i class="fa fa-envelope" style="font-size:48px;color:white"></i>
					</a>
				</div>
			</div>
		</div>
	</div>
</div>
</div>
<script>
	$(document).ready(function(){
		if($(document).width() <= 768){
			var pricing_details_button = $('.pricing-details-button');
			$('.pricing-details-list').hide();
			$( document ).width();
			$(pricing_details_button).click(function(){
				$(this).parents('.pricing-component-body').find('.pricing-details-list').toggle('300');
			})
		}
	})
</script>
