...
class Businessreport_model extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT tbl_shop_orders.id AS order_id,tbl_shop_orders.export_ID, serviceFee,serviceFeeTax, vendor.id, tbl_shop_vendors.serviceFeeTax, date(tbl_shop_orders.updated) AS order_date, tbl_shop_products_extended.vatpercentage AS productVat,tbl_shop_products_extended.`name` AS productName,tbl_shop_products_extended.price,tbl_shop_order_extended.quantity,
				...
				INNER JOIN tbl_shop_vendors ON  vendor.id = tbl_shop_vendors.vendorId
	...
	// public function get_local_report($vendor_id)
	// {
	// 	$query = $this->db->query("SELECT tbl_shop_orders.id AS order_id, tbl_shop_orders.updated AS order_date,serviceFee, tbl_shop_products_extended.vatpercentage AS productVat,tbl_shop_products_extended.`name` AS productName,tbl_shop_products_extended.price,tbl_shop_order_extended.quantity,
	// 		tbl_shop_spot_types.type AS service_type,tbl_shop_products_extended.price * tbl_shop_order_extended.quantity AS AMOUNT,
	// 		((tbl_shop_products_extended.price * tbl_shop_order_extended.quantity) * 100)/(tbl_shop_products_extended.vatpercentage+100) AS EXVAT,
	// 		(tbl_shop_orders.serviceFee)*(tbl_shop_vendors.serviceFeeTax/100) AS EXVATSERVICE,
	// 		tbl_shop_products_extended.price * tbl_shop_order_extended.quantity-tbl_shop_products_extended.price * tbl_shop_order_extended.quantity * 100 / (tbl_shop_products_extended.vatpercentage+100) AS VAT
	// 		FROM tbl_shop_orders INNER JOIN tbl_shop_order_extended ON tbl_shop_orders.id = tbl_shop_order_extended.orderId
	// 		INNER JOIN tbl_shop_products_extended ON tbl_shop_order_extended.productsExtendedId = tbl_shop_products_extended.id
	// 		INNER JOIN tbl_shop_products ON  tbl_shop_products_extended.productId = tbl_shop_products.id
	...
	{
		$query = $this->db->query("SELECT tbl_shop_orders.id AS order_id,tbl_shop_orders.export_ID, date(tbl_shop_orders.updated) AS order_date,tbl_shop_products_extended.deliveryVatpercentage AS productVat,tbl_shop_products_extended.`name` AS productName,tbl_shop_products_extended.price,
	...
	{
		$query = $this->db->query("SELECT tbl_shop_orders.id AS order_id,tbl_shop_orders.export_ID, date(tbl_shop_orders.updated) AS order_date,tbl_shop_products_extended.pickupVatpercentage AS productVat,tbl_shop_products_extended.`name` AS productName,tbl_shop_products_extended.price,
					tbl_shop_order_extended.quantity,
