 ...
 */
class Check424_model extends CI_Model
{
	...
	public function Check424ById($code)
	{
		$this->db->from('tbl_check424');
	...
	function Store424(array $labelInfo)
	{
		try
		{
			...
			do
			{
				$code = 'C-' . substr(str_shuffle($set), 0, 16);
				$this->db->select('id');
				$this->db->from('tbl_check424');
				$this->db->where('code', $code);
				$query = $this->db->get();
			...
			$labelInfo['lastdatetime']= $savedatetime->format('Y-m-d H:i:s');
			$labelInfo['code'] = $code;

			if ($this->db->insert('tbl_check424', $labelInfo))
				$insert_id = $this->db->insert_id();
			else {
				$testquery = $this->db->last_query();
				$insert_id = -1;
...
//				$this->db->select('id, code, lastdatetime');
				$this->db->select('id, code, lastdatetime, answer1, answer2, answer3, answer4, answer5, answer6, answer7, answer8, answer9');
				$this->db->from('tbl_check424');
