<?php
    ...
    Class Shoporder_model_mobile extends AbstractSet_model implements InterfaceCrud_model, InterfaceValidate_model
    {
        public $id;
        public $buyerId;
        public $amount;
		...
		public function fetchOrdersForMobile(string $macNumber): ?array
		{
			$this->load->config('custom');
			$concatSeparator = $this->config->item('concatSeparator');
			$concatGroupSeparator = $this->config->item('contactGroupSeparator');
...
//                                    tbl_shop_printers.macNumber != "' . $macNumber . '"
		...
		public function returnOrders($vendorId): array
		{
			$query = '
	        (
						SELECT
